...
def parse_purchase_date(date_str: str):
    if not date_str:
        return None
...
def get_receipt_age_info(purchase_date_str: str, old_days_threshold: int = 90):
    ...
    if days_old < 0:
        return {
            "is_old": False,
            "days_old": days_old,
            "message": "Receipt date looks like it is in the future. Please check the detected date.",
    ...
    if days_old > old_days_threshold:
        return {
            "is_old": True,
            "days_old": days_old,
            "message": f"Old receipt detected ({days_old} days old). Expiry estimates may not be useful.",
            "purchase_date": purchase_date.strftime("%Y-%m-%d")
        }
